...
public class VectorIndexService {
    ...
    private void insertToMilvus(String content, List<Float> vector, 
        ...
        try {
            ...
            // 生成唯一 ID（使用时间戳 + 哈希值确保唯一性，返回 Long 类型）
            String source = (String) metadata.get("_source");
            long id = Math.abs((source + "_" + chunkIndex).hashCode()) * 1000000L + System.currentTimeMillis() % 1000000;

            // 构建字段数据
            List<InsertParam.Field> fields = new ArrayList<>();
            
            // ID 字段 - 使用 Long 类型匹配 Milvus Int64 字段
